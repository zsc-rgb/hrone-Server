...
| 第6阶段 | 安全认证 | 2天 | ✅ 已完成 | 2025-11-11 |
| 第7阶段 | 权限系统 | 1天 | ✅ 已完成 | 2025-11-24 |
| 第8阶段 | API接口开发 | 1-2天 | ✅ 已完成 | 2025-11-24 |
| 第9阶段 | 高级特性 | 2-3天 | ⏳ 待开始 | - |
...
- [x] 理解RBAC权限模型（用户-角色-菜单-权限）
- [x] 能实现接口权限验证（@RequiresPermissions + 切面，结合 JWT 登录信息和请求级缓存）
- [x] 能实现数据权限过滤（@DataScope：支持全部/自定义/本部门/本部门及以下/仅本人五种范围）
- [x] 掌握用户-角色-菜单/角色-部门关联查询与AOP结合落地的数据权限策略

### 第8阶段：API接口开发

- [x] 能规范地设计RESTful接口（字典/日志/上传模块）
- [x] 能实现文件上传下载（`/common/file/upload` + 本地存储服务）
- [x] 能实现操作日志记录（@OperLog + AOP 入库）
- [x] 掌握接口文档同步与阶段总结
...
| 日期 | 学习内容 | 学习时长 | 完成度 | 备注 |
|-----|---------|---------|--------|------|
| 2025-11-24 | 第7-8阶段：权限&API接口 | 8小时 | 100% | 权限注解升级、字典、文件上传、日志管理 |
