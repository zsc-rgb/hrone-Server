...
**开始时间：** 2025-11-11  
**当前进度：** 6.1~6.4 全部完成，登录、验证码、JWT、权限注解联动运行
...
#### 8.1 系统管理接口 ✅
- [x] 新增 `SysDictType` / `SysDictData` 实体、Mapper、Service、Controller
- [x] 完整的字典类型 + 字典数据 CRUD，以及按类型查询
- [x] 为用户/角色/部门/菜单接口补充 `@OperLog` 注解，联动操作日志切面

#### 8.2 文件上传 ✅
- [x] `FileStorageService` 封装本地磁盘存储，支持模块化子目录
- [x] `CommonFileController /common/file/upload` 单文件上传接口
- [x] 配置化上传目录（`hrone.file.upload-dir`）+ Spring Multipart 限制

#### 8.3 日志管理 ✅
- [x] `SysOperLog`、`SysLoginLog` 表结构、实体、Mapper、Service、Controller
- [x] 新增 `@OperLog` 注解 + `OperLogAspect`，自动记录操作日志
- [x] 登录接口写入成功/失败登录日志，可查询、批量删除、清空
- [x] 文档同步，README/PROJECT_PROGRESS/学习计划 全量记录第8阶段成果
...
- 完成第7阶段：权限系统
  - 权限切面改为真实 RBAC 校验
  - 数据权限支持全部/自定义/本部门/本部门及以下/仅本人
  - 新增 `SysRoleDeptMapper`、`DataScopeContext`
- 完成第8阶段：API接口开发
  - 字典类型+数据模块（实体/Mapper/Service/Controller/SQL）
  - 文件上传接口 + FileStorageService
  - 操作日志注解+切面、登录日志记录与查询接口

**学习时长：** 8小时

**收获：**
- 进一步理解了 AOP 与日志审计的结合方式
- 学会了通过注解驱动的操作日志记录
- 掌握了上传目录抽象和多模块共享服务的设计
- 熟悉了“控制器 -> Service -> Mapper -> SQL”一整套增量开发流程
